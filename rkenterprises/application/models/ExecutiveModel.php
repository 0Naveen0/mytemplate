 ...
 * @author Naveen Kamal 
 */
class ExecutiveModel extends CI_Model {
    function __construct() {
     ...
     public function getExecutiveName($executiveid){
      ...
      public function getAllExecutive(){
         $field = 'financeexecutive';
            $order = 'ASC';
      
        $displayUsers = array();
        $query = $this->db->select('financeexecutiveid,financeexecutive')
                ->from('financeexecutive')
                //->where('role !=', 'Administrator')
               ->order_by($field, $order)
                ->get();
        
        if ($query->num_rows() === 0) {
            $displayUsers = array('executiveid'=>"NULL",'executivename'=>'Not Found');
            ...
            foreach ($query->result() as $row) {
                ...
                $data=array('executiveid'=>$row->financeexecutiveid,'executivename'=>$row->financeexecutive);
                $displayUsers[$i++]=$data;
