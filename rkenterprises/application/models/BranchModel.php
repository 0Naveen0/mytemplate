...
->order_by('transactiondate')->get();
      ...
      if ($accountquery->num_rows() === 0) {
            ...
            $displayUsers.="<table class='table table-striped table-bordered name='customerLedgerTable''>";
            ...
            //echo $remark;
                if($row->type==="debit"){
                    ...
                    $debitsum=$debitsum+($row->amount);
                     $displayUsers.="<tr><td><button type='button' class='tid' id='".$row->id."' value='".$row->id."'>" . $row->id . "</button></td><td>" .$date . "</td>". "<td>" .$row->amount  . "</td><td>" ."0"  . "</td><td>" . $remark . "</td><td>" . $row->comment . "</td></tr>";
                }else if($row->type==="credit"){
                    
                   $balance=$balance-($row->amount); 
                    $creditsum=$creditsum+($row->amount);
                     $displayUsers.="<tr><td><button type='button' class='tid' id='".$row->id."' value='".$row->id."'>" . $row->id . "</button></td><td>" .$date  . "</td>". "<td>" . "0"  . "</td><td>" . $row->amount . "</td><td>" . $remark . "</td><td>" . $row->comment . "</td></tr>";
                }
             }
		...
		//	echo $displayUsers;
           // return $displayCustomerData;
            ...
            foreach ($mainquery->result() as $row) {
                ...
                $data=array('cid'=>$row->Cid,'customername'=>$row->Name,'debit'=>$row->Debit,'credit'=>$row->Credit,'balance'=>$row->Balance);
...
//	return $displayUsers;
     ...
     public function getBranchLocation($branchid){
      ...
      public function getBranchName($branchid){
          ...
          $locationquery=$this->db->select('branchname')
                                    ->from('branch')
                                    ->where('branchid',$branchid)
                                    ->get();
             if ($locationquery->num_rows() === 1) { 
