 ...
 * @author Naveen Kamal 
 */
class HypothecationModel extends CI_Model {
    function __construct() {
     ...
     public function getFinancerName($financerid){
      ...
      public function getAllFinancer(){
         $field = 'hypothecation';
            $order = 'ASC';
      
        $displayUsers = array();
        $query = $this->db->select('hypothecationid,hypothecation')
                ->from('hypothecation')
        ...
        if ($query->num_rows() === 0) {
            $displayUsers = array('financerid'=>"NULL",'financername'=>'Not Found');
            ...
            foreach ($query->result() as $row) {
                ...
                $data=array('financerid'=>$row->hypothecationid,'financername'=>$row->hypothecation);
                $displayUsers[$i++]=$data;
